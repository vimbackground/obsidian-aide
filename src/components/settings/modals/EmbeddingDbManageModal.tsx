...
function EmbeddingDbManageModalComponent() {
    ...
    try {
      ...
      await (
        ...
        embeddingModel,
        {
          ...
          excludePatterns:
            settings.ragOptions.filterMode === 'whitelist'
              ? []
  ...
  return (
    <div className="aide-settings-embedding-db-manage-root">
      <div className="aide-settings-embedding-db-manage-header">
        <button
        ...
        <span className="aide-settings-embedding-db-manage-last-updated">
          Last updated: {dayjs(dataUpdatedAt).format('YYYY-MM-DD HH:mm:ss')}
        </span>
      </div>
      <table className="aide-settings-embedding-db-manage-table">
        <thead>
          <tr>
            <th>嵌入模型</th>
            <th>已嵌入分块数</th>
          ...
          {stats.map((stat) => (
            <tr key={stat.model}>
              ...
              {indexProgressMap.get(stat.model) ? (
                <td className="aide-settings-embedding-db-manage-actions-loading">
                ...
                <td className="aide-settings-embedding-db-manage-actions">
                  <button
                    className="clickable-icon"
