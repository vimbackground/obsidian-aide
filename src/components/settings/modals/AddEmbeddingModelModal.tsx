...
type AddEmbeddingModelModalProps = {
  plugin: SmartComposerPlugin
  onClose: () => void
}

export class AddEmbeddingModelModal extends ReactModal<AddEmbeddingModelModalProps> {
  constructor(app: App, plugin: SmartComposerPlugin) {
    super({
      app: app,
      Component: AddEmbeddingModelComponent,
      props: { plugin },
      options: {
        title: '添加自定义嵌入模型',
      },
...
function AddEmbeddingModelComponent({
  plugin,
  onClose,
}: AddEmbeddingModelModalProps) {
  ...
  const handleSubmit = async () => {
    ...
    if (plugin.settings.embeddingModels.some((m) => m.id === formData.id)) {
      new Notice('已存在相同 ID 的嵌入模型，请修改模型标识 (ID)')
      return
    }

    setIsSubmitting(true)
    new Notice('正在测试并连接嵌入模型以获取向量维度...')
    ...
    try {
      ...
      new Notice(`已成功添加嵌入模型 "${formData.id}" (维度: ${dimension})`)
      onClose()
    } catch (error) {
      new Notice(
        error instanceof Error ? error.message : '添加嵌入模型时发生未知错误',
      )
    } finally {
  ...
  return (
    <>
      ...
      {availableModels && availableModels.length > 0 && availableModels.some(isEmbeddingModel) && (
        <ObsidianSetting
          name="只显示识别出的嵌入模型"
          desc={`已智能筛选 ${availableModels.filter(isEmbeddingModel).length} 个嵌入模型。如需查看服务商所有 ${availableModels.length} 个模型，可关闭此开关。`}
        >
          <ObsidianToggle
            value={!showAllModels}
      ...
      >
        {availableModels && availableModels.length > 0 ? (
          ...
          <ObsidianTextInput
            value={formData.model}
            placeholder="输入嵌入模型名称（如 BAAI/bge-m3 或 text-embedding-3-small）"
            onChange={(value: string) =>
      ...
      <ObsidianSetting
        name="输出维度 (Output Dimensions)"
        desc="可选。仅当模型支持自定义维度输出（如 OpenAI text-embedding-3 系列）时填写。留空则自动检测并使用模型的默认维度。"
      >
        <ObsidianTextInput
          value={outputDimensionInput}
          placeholder="留空自动检测（如 1024 或 1536）"
          onChange={(value: string) => {
      ...
      <ObsidianSetting>
        <ObsidianButton 
          text={isSubmitting ? '正在验证添加...' : '添加嵌入模型 (Add)'} 
