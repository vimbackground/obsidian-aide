...
- "env": (Optional) Key-value pairs of environment variables`}
        className="aide-settings-textarea-header aide-settings-description-preserve-whitespace"
        required
      />
      <TextareaAutosize
        value={parameters}
        ...
        className="aide-mcp-server-modal-textarea"
        maxRows={20}
        minRows={PARAMETERS_PLACEHOLDER.split('\n').length}
      />
      {validationError !== null ? (
        <div className="aide-mcp-server-modal-validation aide-mcp-server-modal-validation--error">
          {validationError}
        </div>
      ) : (
        <div className="aide-mcp-server-modal-validation aide-mcp-server-modal-validation--success">
          Valid parameters
        </div>
      )}
