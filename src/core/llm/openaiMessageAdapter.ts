    ...
    switch (message.role) {
      case 'user': {
        ...
        if (Array.isArray(message.content)) {
          // If all parts are text, flatten into a single string to avoid triggering proxy multimodal/base64 decoding errors
          ...
          if (!hasImage) {
            ...
            content = message.content.map((part): ChatCompletionContentPart => {
              switch (part.type) {
                case 'text':
