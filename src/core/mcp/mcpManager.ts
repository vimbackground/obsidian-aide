...
export class McpManager {
  ...
  private async connectServer(
    ...
    // Windows 平台命令适配：如果命令为 npx 或 npm，在 Windows 上自动使用 .cmd
    if (Platform.isWin && !command.toLowerCase().endsWith('.cmd') && !command.toLowerCase().endsWith('.exe')) {
      if (command.toLowerCase() === 'npx' || command.toLowerCase() === 'npm') {
        command = `${command}.cmd`
      }
    }
    ...
    Object.assign(cleanEnv, this.defaultEnv, serverParams.env ?? {})
  ...
  public async listAvailableTools(): Promise<McpTool[]> {
    ...
    if (this.availableToolsCache) {
      return [...builtinTools, ...this.availableToolsCache]
    }
    ...
    return [...builtinTools, ...availableTools]
  }
