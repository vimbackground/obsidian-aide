...
/**
 * 跨平台将 output/obsidian-aider 产物压缩打包为与 Obsidian 兼容的 output/aider.zip
 * 用法: node scripts/package-zip.js
 */

const projectRoot = path.resolve(__dirname, '..');
const outputDir = path.join(projectRoot, 'output', 'obsidian-aider');
const zipFile = path.join(projectRoot, 'output', 'aider.zip');
const legacyZipFile = path.join(projectRoot, 'output', 'obsidian-aider.zip');
...
// 清理旧版可能残留的冗余 zip 文件
if (fs.existsSync(legacyZipFile)) {
  fs.unlinkSync(legacyZipFile);
}

console.log(`[Package Zip] 开始打包 output/obsidian-aider -> output/aider.zip ...`);
...
try {
  ...
  if (fs.existsSync(zipFile)) {
    ...
    console.log(`[Package Zip] 打包成功: output/aider.zip (${sizeMB} MB, ${stat.size} bytes)`);
  } else {
